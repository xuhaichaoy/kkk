...
export const categoriesAtom = atomWithStorage<string[]>(CATEGORY_STORAGE_KEY, []);
export const filterAtom = atom<TodoFilterState>(defaultFilterState);
export const calendarMonthAtom = atom<Date>(new Date());

export const addTodoAtom = atom(false, (get, set, task: Omit<TodoTask, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  console.log('➕ 新建任务:', {
    ...
    dueDateValid: newTask.dueDate ? new Date(newTask.dueDate).toString() : 'no date',
    reminderValid: newTask.reminder ? new Date(newTask.reminder).toString() : 'no reminder'
  });
  
  set(todosAtom, [newTask, ...existing]);
});

export const updateTodoAtom = atom(false, (get, set, update: { id: string; changes: Partial<TodoTask> }) => {
  const existing = get(todosAtom);
  console.log('🔄 更新任务:', {
    updateId: update.id,
  ...
  console.log('✅ 任务更新结果:', {
    ...
    dueDateValid: updatedTodo?.dueDate ? new Date(updatedTodo.dueDate).toString() : 'no date',
    reminderValid: updatedTodo?.reminder ? new Date(updatedTodo.reminder).toString() : 'no reminder'
  });
...
export const removeTodoAtom = atom(null, (get, set, id: string) => {
  const existing = get(todosAtom);
  set(todosAtom, existing.filter(todo => todo.id !== id));
});
...
export const removeTagAtom = atom(null, (get, set, tag: string) => {
  set(tagsAtom, get(tagsAtom).filter(item => item !== tag));
  set(
    todosAtom,
...
export const removeCategoryAtom = atom(null, (get, set, category: string) => {
  set(categoriesAtom, get(categoriesAtom).filter(item => item !== category));
  set(
    todosAtom,
    get(todosAtom).map(todo => ({
      ...todo,
